...
ui.atom = {
    ...
    drawerBase: (opts = {}) => {
        ...
        const drawerStyle = `position: absolute; bottom: 0; left: 0; width: 100%; height: ${height}; box-shadow: var(--shadow-lg); transition: transform 0.3s cubic-bezier(0.25, 1, 0.5, 1); pointer-events: auto; display: flex; flex-direction: column; z-index: 201; border-top: 1px solid var(--border); transform: translateY(${isOpen ? '0%' : '100%'}); ${style}`;
        const dataAttrs = toggleAction ? ` data-action="${toggleAction}" data-val="${!isOpen}"` : '';
        const handleStyle = `position: absolute; right: 0; bottom: 100%; color: inherit; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; font-weight: bold; box-shadow: var(--shadow-sm); user-select: none; width: 60px; height: 40px; border-radius: 8px 8px 0 0; border: 1px solid var(--border); border-bottom: none; pointer-events: auto; z-index: 202; background: inherit;`;
...
// ============================================================================
// LAYER 3: Composers (組合器) - 徹底汰除 --current 舊變數，全面擁抱標準變數
// ============================================================================
ui.composer = {
    pageHeader: (opts = {}) => {
        return ui.atom.headerBase({ ...opts, style: `background: var(--bg-card); color: var(--text); border-bottom: 1px solid var(--border); ${opts.style || ''}` });
    },
    qtySelector: (opts = {}) => {
        return ui.atom.qtySelectorBase({ ...opts, style: `background: var(--bg-box); color: var(--text); border: 1px solid var(--border); ${opts.style || ''}` });
    },
    formField: (opts = {}) => {
    ...
    drawer: (opts = {}) => {
        ...
        return ui.atom.drawerBase({ ...opts, className: themeClass, style: `background: var(--bg-card); color: var(--text); border-top: 1px solid var(--border); ${opts.style || ''}` });
    },
    cardVertical: (opts = {}) => {
        ...
        return ui.atom.cardBase({ action: opts.action, actionId: opts.actionId, style: `background: var(--bg-card); border-color: var(--border-card); color: var(--text); ${opts.style || ''}`, children: `${mediaHtml}<div class="card-info-area" style="text-align:center; margin-bottom:10px;"><div style="font-weight:bold; color:var(--text);">${opts.title}</div>${opts.subTitle !== undefined ? `<div style="font-size:0.8rem; opacity:0.8; margin-top:2px; font-weight:bold;">${opts.subTitle}</div>` : ''}${opts.stock !== undefined ? `<div style="font-size:0.75rem; opacity:0.5; margin-top:2px;">庫存: ${opts.stock}</div>` : ''}</div><div style="width:100%;">${opts.actionBtnHtml || ui.atom.buttonBase({label:'購買', style:'width:100%;'})}</div>` });
    },
    centeredModalBody: (opts = {}) => {
        const { icon, title, desc, extraHtml = '' } = opts;
        return `<div style="text-align:center; padding:10px;"><div style="font-size:3.5rem; margin-bottom:10px; filter:drop-shadow(var(--shadow-sm));">${icon}</div><h3 style="margin-bottom:5px; color:var(--text);">${title}</h3>${desc ? `<p style="color:var(--text-2); font-size:0.9rem;">${desc}</p>` : ''}${extraHtml ? `<div style="margin-top:15px;">${extraHtml}</div>` : ''}</div>`;
    },
    closeModal: (id) => {
...
// ============================================================================
// LAYER 4: Smart Components (智慧元件) - 已清除硬編碼顏色
// ============================================================================
ui.smart = {
    taskCard: (t, isHistory = false) => {
        ...
        let badges = '';
        // 🌟 修正：將 #ef6c00 改為 var(--color-warning)
        ...
        if (!isHistory) {
            if (t.type === 'count') {
                ...
                for (let i = 1; i <= t.subs.length; i++) {
                    // 🌟 修正：將 rgba(0,0,0,0.1) 改為 var(--bg-track)
                    stepsHtml += `<div style="width:12px; height:12px; background:${i <= doneCount ? 'var(--color-correct)' : 'var(--bg-track, rgba(0,0,0,0.1))'}; border-radius:50%; flex-shrink:0; z-index:2; transition:background 0.3s;"></div>`;
                    if (i < t.subs.length) stepsHtml += `<div style="flex:1; height:4px; background:${i < doneCount ? 'var(--color-correct)' : 'var(--bg-track, rgba(0,0,0,0.1))'}; margin:0 -2px; z-index:1; transition:background 0.3s;"></div>`;
                }
        ...
        if (t.type === 'count') {
            ...
            // 🌟 修正：將 #fff 改為 var(--text-inverse, #fff)
            subtaskHtml = `<div style="margin-top:8px; border-top:1px dashed var(--border); padding-top:8px; cursor:default;" data-stop="true">` + t.subs.map((sub, idx) => `<div style="display:flex; align-items:center; gap:8px; margin-bottom:6px; cursor:pointer; padding:6px 4px; border-radius:4px; transition:background 0.2s;" data-action="toggleSubtask" data-id="${t.id}" data-val="${idx}"><div class="chk ${sub.done?'checked':''}" style="width:18px; height:18px; border:1px solid ${sub.done?'var(--color-correct)':'var(--text-ghost)'}; border-radius:4px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">${sub.done ? '<span style="color:var(--text-inverse, #fff); font-size:12px; font-weight:bold;">✓</span>' : ''}</div><div style="font-size:0.9rem; ${sub.done?'text-decoration:line-through; opacity:0.5;':''} flex:1; user-select:none;">${sub.text}</div></div>`).join('') + `</div>`;
        }
    ...
    shopItem: (item, editBtnHtml = '') => {
        ...
        // 🌟 修正：將 rgba(255, 215, 0, 0.1) 換成 var(--color-gold-soft)
        const specialStyle = isOnce ? 'border: 2px solid var(--color-gold); background: linear-gradient(180deg, var(--color-gold-soft) 0%, transparent 100%); box-shadow: inset 0 2px 10px var(--color-gold-soft);' : '';
        
        const soldOutOverlay = isSoldOut ? `<div style="position:absolute; top:0; left:0; width:100%; height:100%; background:var(--bg-card); opacity:0.75; z-index:3; pointer-events:none;"></div><div style="position:absolute; top:0; left:0; width:100%; height:100%; z-index:4; display:flex; align-items:center; justify-content:center; pointer-events:none;"><div style="border: 3px solid var(--color-danger); color: var(--color-danger); font-weight: bold; font-size: 1.2rem; padding: 5px 10px; transform: rotate(-15deg); border-radius: 8px; background:var(--bg-panel); box-shadow:var(--shadow-sm);">SOLD OUT</div></div>` : '';
    ...
    wardrobeItem: (item, isWearing, isUnlocked, isPreviewing, actionBtnHtml) => {
        ...
        // 🌟 修正：陰影從 rgba 改為 var(--shadow-sm)
        return `<div class="avatar-card" style="min-width:110px; height:165px; flex-shrink:0; border-radius:var(--radius-md); display:flex; flex-direction:column; padding:10px; transition:all var(--t-base); border: 1.5px solid transparent; ${borderStyle}"><div data-action="previewAvatarItem" data-id="${item.id}" style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; cursor:pointer;"><img src="img/${item.id}.png" data-fallback="true" style="height:65px; object-fit:contain; filter:drop-shadow(var(--shadow-sm));"><div class="img-fallback" style="display:none; font-size:2.5rem">👕</div><div style="font-size:0.8rem; margin-top:8px; font-weight:bold; color:var(--text-2); text-align:center;">${item.name}</div></div><div style="margin-top:5px; width:100%;">${actionBtnHtml}</div></div>`;
    }
};
...
view.getPriorityInfo = (importance, urgency) => {
    ...
    if (urgency >= 3) return { color: 'var(--color-warning)', label: '⚡ 突發', border: 'var(--color-warning)' };
...
view.renderMain = () => {
    ...
    const quickIcons = window.SQ.ThemeConfig ? window.SQ.ThemeConfig.getQuickIcons(isBasic) : [];
    
    const quickButtonsHtml = quickIcons
        .map(b => `<div data-action="${b.action}" ${b.val ? `data-val="${b.val}"` : ''} style="${btnStyle3D}">${b.icon}</div>`)
        .join('');

    // 🌟 修正：角色的陰影從 rgba(0,0,0,0.3) 改為 var(--shadow-md)
    let charImg = window.Assets?.getCharImgTag
        ? window.Assets.getCharImgTag('main-char-img', 'height: 100%; width: auto; object-fit: contain; filter: drop-shadow(var(--shadow-md));')
        : '<div style="font-size:6rem;">🦸</div>';

    const storyBtn = !isBasic
        ? ui.atom.buttonBase({
            label: '🌀 進入劇情模式', theme: 'correct', action: 'emit', actionVal: 'action_enter_story_mode',
            style: `width: 240px; padding: 12px; border-radius: var(--radius-full); font-size: 1.1rem;`
          }).replace('data-val="action_enter_story_mode"', 'data-event="action_enter_story_mode"')
        : '';
    ...
    container.innerHTML = `
        <div style="position: relative; width: 100%; height: 100%; overflow: hidden;">
            ...
            <div style="width: 100%; height: 75%; display: flex; align-items: flex-end; justify-content: center; transform: translateY(40px); position: relative; z-index: 10; pointer-events: none;">
                ...
                <div data-action="navigate" data-val="stats" style="position: absolute; bottom: 0; left: 30%; width: 40%; height: 90%; cursor: pointer; pointer-events: auto; z-index: 20;"></div>
            </div>
...
view.renderNavbar = (targetPageId) => {
    ...
    // 🌟 直接從 Config 抓取 Navbar，刪除 hardcode 陣列
    const navItems = window.SQ.ThemeConfig ? window.SQ.ThemeConfig.getNavbar() : [];

    const activeId = targetPageId
        || window.SQ.Temp?.currentView
        || (document.querySelector('.page.active')?.id.replace('page-', ''))
        || 'main';

    container.innerHTML = navItems.map(item =>
        `<button class="nav-item ${item.id === activeId ? 'active' : ''}" id="nav-${item.id}"
            data-action="${item.action}" ${item.val ? `data-val="${item.val}"` : ''}>
